...
def plot_decision_boundary(model, X, y):
    ...
    plt.title('Neural Network Decision Boundary - Blobs')
    plt.colorbar(label='Prediction')
    plt.show()


def train(model, X, y, learning_rate=1.0, num_iterations=150, target_accuracy=0.99):
...
if __name__ == "__main__":
    # Generate dataset - Gaussian blobs
    X, y = make_blobs(n_samples=100, centers=2, cluster_std=1.5, random_state=42)

    # Normalize - blobs have large feature values, tanh needs ~[-2, 2]
    ...
    # Create model - simpler architecture for linearly separable data
    random.seed(17)
    model = MLP(2, [8, 1])
